...
void fillGraph (Graph g, Vertex* vertices) {
	...
	for(gIndex = 0; gIndex < g->nV; gIndex++) {
		...
		if (g->edges[gIndex] == NULL) {
			...
			//printf("empty slot at gIndex = %d -- adding key '%s'\n", gIndex, vertices[vIndex]);
			List newL = newList(vertices[vIndex], NULL);
			g->edges[gIndex] = newL;
			//printf("edges[%d]: '%s' -> %p\n", gIndex, newL->key, newL->first);
			vIndex ++;
		}
	}
}
...
void insertEdge(Graph g, Edge e) { 
	...
	if (!inLL(g->edges[startI]->first, e.end)) {   // edge e not in graph
		...
		//g->edges[endI] = insertLL(g->edges[endI], e.start); TODO
		g->nE++;
	}
}
...
/*
// converts the adjacency list into an array of keys
Vertex* keysToArray(Graph g) {
	Vertex* keyArray = calloc(g->nV, sizeof(Vertex*));
	// traverse adjacecny list
	int i;
	for (i = 0; i < g->nV; i++) {
		// access the listrep
		// dump into array
		keyArray[i] = g->edges[i]->key;
	}
	
	return keyArray;
}
double* prsToArray(Graph g) {
	return 0;
}
*/
