// Altered version of Map.c from 2521 wk07 lab
...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	int count = 0; 
	int j = 0;
    struct vNode *loc = NULL;
    loc = g->connections[start];
	while (loc != NULL && j < MAX_TRANSPORT) {
        if (loc->v == end) {
			//printf("	%d is connected to %d\n", start, loc->v);
			type[j] = loc->type;
	        //printf("	type of transport: %d\n", loc->type);
	        j++;
	        count++;
        }
        // if isSea,
        else if (isSea(loc->v) == 1) {
            // go to that sea and check if end is connected
            struct vNode* loc1 = g->connections[loc->v];
            while (loc1 != NULL) {
				if (loc1->v == end) {
					type[j] = loc->type;
					j++;
